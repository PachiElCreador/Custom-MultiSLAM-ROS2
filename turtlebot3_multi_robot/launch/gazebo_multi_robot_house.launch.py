...
def generate_launch_description():
    ld = LaunchDescription() #object to store launch actions

    # Define the names and poses of the robots
    robots = [
        {'name': 'tb1', 'x_pose': '-4', 'y_pose': '4', 'z_pose': 0.01},
        {'name': 'tb2', 'x_pose': '-1', 'y_pose': '1', 'z_pose': 0.01},
        #{'name': 'tb3', 'x_pose': '-4', 'y_pose': '1', 'z_pose': 0.01},
        #{'name': 'tb4', 'x_pose': '-1', 'y_pose': '4', 'z_pose': 0.01},
        # ...
        # ...
        ]

    TURTLEBOT3_MODEL = 'burger' #model to use
    
    #Define and declare arguments fot use in the launch file 
    ...
    enable_rviz = LaunchConfiguration('enable_rviz', default='true') #False to disallow opening rviz mapping and visualization
    declare_enable_rviz = DeclareLaunchArgument(
        name='enable_rviz', default_value=enable_rviz, description='Enable rviz launch'
    )

    #Get package path for resources
    ...
    launch_dir = os.path.join(package_dir, 'launch', 'bringup')

    rviz_config_file = LaunchConfiguration('rviz_config_file')
    
    # Rviz configuration file settings
    ...
    # Define paths to the URDF and world files
    urdf = os.path.join(
    ...
    # Gazebo server an client to launch the specified world
    ...
    # Declare a navigation parameters file.
    ...
    # Add the declared argument and commands to the launch description
    ...
    # Define remappings fot the transform topics
    # ATENCION
    remappings = [('/tf', 'tf'), ('/tf_static', 'tf_static')]
    
    # Node to load a static map using the map server
    map_server=Node(package='nav2_map_server',
        executable='map_server',
        name='map_server',
        output='screen',
        #parameters=[{'yaml_filename': os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'maps', 'turtlebot3_house_map.yaml')}], map_house.yaml
    ...
    # Lifecycle manager to manage the lifecycle of the map server node.
    # ATENCION
    ...
    # Add the map server nodes to the launch description
    ...
    # variable to manage robot spawn
    last_action = None
    
    # Spawn turtlebot3 instances in gazebo
    for robot in robots:
        # namespace of the robot spawned
        ...
        # Include navigation bringup for each robot
    ...
    ######################
    # Launch Rviz once last robot spawns
    for robot in robots:
